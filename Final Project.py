...
for x in orig_numeric_cols:
    median = X_train_imp[x].median()
    X_train_imp[x].fillna(median, inplace=True)
    X_test_imp[x].fillna(median, inplace=True)   
...
# TODO: Do this for the training and test data separately.
for x in orig_categorical_cols:
    mode = X_train_imp[x].mode(dropna=True)
    if not mode.empty:
        mode = mode[0]
        X_train_imp[x].fillna(mode, inplace=True)
        X_test_imp[x].fillna(mode, inplace=True)
...
#There is only one column with question marks, meaning that it is the only column with missing values. Therefore we cannot 
#use both manual filling and filling with scikit learn, so only one of them will work. 
# IMPUTE_EXAMPLE_COL = "stalk-root"  # TODO: change if needed
# if IMPUTE_EXAMPLE_COL in X_train_imp.columns:
#     cat_imp = SimpleImputer(strategy="most_frequent")
#     X_train_imp[IMPUTE_EXAMPLE_COL] = cat_imp.fit_transform(X_train_imp[[IMPUTE_EXAMPLE_COL]])
#     X_test_imp[IMPUTE_EXAMPLE_COL]  = cat_imp.transform(X_test_imp[[IMPUTE_EXAMPLE_COL]])
...
remaining = X_train_enc.select_dtypes(exclude=["number"]).columns.tolist()

# TODO: Apply get_dummies to the remaining categoricals (fit on TRAIN, align TEST) (1 lines of code)
X_train_enc = pd.get_dummies(X_train_enc, columns=remaining)
X_test_enc  = pd.get_dummies(X_test_enc,  columns=remaining)
